...
function expandEvent(r_oDiv) {
	//alert(r_oDiv.offsetHeight + ',' + r_oDiv.offsetWidth);
	//alert(r_oDiv.firstChild.offsetWidth);
	m_lOldWidth = parseInt(r_oDiv.offsetWidth) - 0;
	m_lOldHeight = parseInt(r_oDiv.offsetHeight);
	if (m_lOldWidth < EXPAND_SIZE || m_lOldHeight < EXPAND_SIZE) {
		with (r_oDiv.style) {
			if (m_lOldWidth < EXPAND_SIZE) { width = EXPAND_SIZE + "px"; }
			if (m_lOldHeight < EXPAND_SIZE) { height = EXPAND_SIZE + "px"; }
			backgroundColor = "#ffffff";
			borderRight = '1px solid #8f8daf';
			borderLeft = '1px solid #8f8daf';
			borderBottom = '1px solid #8f8daf';
			zIndex = 5;
			cursor = "hand";
		}
	}
	//r_oDiv.style.top = "100px";
	//alert(r_oDiv.style.top);
}
function collapseEvent(r_oDiv) {
	with (r_oDiv.style) {
		width = m_lOldWidth;
		height = m_lOldHeight;
		zIndex = 0;
	...
	//r_oDiv.style.backgroundColor = "#ffffff";
}
